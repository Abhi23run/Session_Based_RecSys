...
class item_LSTM(nn.Module):
    ...
    def compute_metric(self,test,test_users,test_items,test_contexts):
        ...
        hits5_item, MRR5_item,MRR10_item = 0,0,0 
        ...
        with torch.no_grad():
            for iteration in range(int(test_num / self.batch_size) + 1):
                ...
                for i in range(ed_idx-st_idx):
                    ...
                    hits5_item += (1 if rank <= 5 else 0)
                    
                    HITS_item += (1 if rank <= 10 else 0)
                    
                    MRR5_item += (1/rank if rank <= 5 else 0)
                    MRR10_item += (1/rank if rank <= 10 else 0)
        ...
        return MRR_item,HITS_item, hits5_item, MRR5_item, MRR10_item
    ...
    def traintest(self, train, val,test, epochs):
        ...
        patience = 5
        best_loss,best_MRR,best_HITS, best_MRR5, best_MRR10,best_HITS5 = 0,0,0,0,0,0
        ...
        for epoch in range(epochs):
            ...
            if epoch % 1 == 0:
                ...
                #return MRR_item,HITS_item, hits5_item, MRR5_item, MRR10_item
                MRR_item_val,HITS_item_val,HITS5_item_val, MRR5_item_val, MRR10_item_val = self.compute_metric(val,val_users,val_items,val_contexts)
                MRR_item,HITS_item, HITS5_item, MRR5_item, MRR10_item = self.compute_metric(test,test_users,test_items,test_contexts)
                ...
                if best_loss<HITS_item_val:
                    ...
                    best_MRR5, best_HITS5, best_MRR10 = MRR5_item, HITS5_item, MRR10_item
                elif epoch-best_epoch>=patience:
                    break
                start_time = time.time()

        print("[Next-item Prediction]\tEpoch {}\tTest_MRR = {}\tTest_Recall@10 = {}".format(best_epoch, best_MRR,best_HITS))
        return best_MRR, best_HITS, best_MRR5, best_HITS5, best_MRR10
