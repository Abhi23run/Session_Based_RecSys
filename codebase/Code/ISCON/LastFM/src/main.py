...
def parse_args():
    ...
    parser.add_argument('--data_name', type=str, default="data/lastfm.tsv",
                        help='Data name.')
...
def session_find_and_split(raw_data, train_ratio,seed):
    ...
    for user in sessions.keys():
        ...
        rat = round(train_ratio * L)
        
        trsesh = session_list[:rat]
        tesesh = session_list[rat:]

        trainsessions = []
        testsessions = []
        cutoff = train_ratio*len(items_per_user[user])
        for (i, session) in enumerate(trsesh):
             sesh = session[2]
             trainsessions.append(sesh)
        for (i, session) in enumerate(tesesh):
             sesh = session[2]
             testsessions.append(sesh)
        
        for (i, session) in enumerate(session_list):
            items = session[0]
            times = session[1]
            sesh = session[2]
            if sesh in trainsessions:
                 train_sessions.append(int(counter))
                 for (idx,item) in enumerate(items):
                     x = int(counter)
                     y = int(session_num + item)
...
def context_assignment(G_train, G_all, emb_dim, session_num, train_sessions, test_sessions,seed,args):
    ...
    print(len(all_embds))
    val = np.array([node-session_num for (idx,node) in enumerate (G_all.nodes()) if node >= session_num])
    #print(sorted(val))
...
def context_prediction(sessions,items_per_user, look_back, emb_dim, C, session_embds, item_embds, labels, centers, device, seed, args):
    ...
    # final_predicted_contexts,best_MRR,best_mrr10, best_mrr5,best_HITS,best_HITS10

    predicted_contexts,MRR,mrr10,mrr5,HITS,HITS10 = context_predictor.traintest(train=train, val=val,test=test, epochs=args.epochs,predicted_contexts = predicted_contexts)
    return predicted_contexts,MRR,mrr10,mrr5,HITS,HITS10
...
def item_prediction(sessions,items_per_user, look_back, emb_dim, C, item_embds, labels, predicted_contexts, device, seed, args):
    ...
    #best_MRR, best_HITS, best_MRR5, best_HITS5, best_MRR10
    MRR,HITS,mrr5,hits5,mrr10 = item_predictor.traintest(train=train, val=val,test=test, epochs=args.epochs)
    return MRR, HITS,mrr5,hits5,mrr10
...
def main():
    ...
    X = pd.read_csv('data/lastfm.tsv', sep='\t')
    X.drop("Unnamed: 0", inplace = True, axis = 1)
    print(X.head(5))
    X.columns = ["user","item","time"]

    nsplitn = 10
    runnodes = round( max( X["user"] ) / nsplitn)
    X = X[X["user"] <= runnodes]
    X = X.values
    ...
    MRR5, MRR10, HITS5,HITS10 = [],[],[],[]
    
    MRR10_contexts, MRR5_contexts = [], []
    
    
    MRR_contexts,HITS_contexts,HITS10_contexts,MRR5_contexts,MRR10_contexts = [],[],[],[],[]
    MRR,MRR5,MRR10,HITS5,HITS =[],[],[],[],[]
    ...
    for i in range(num_repeats):
        ...
        #return predicted_contexts,MRR,mrr10,mrr5,HITS,HITS10
        predicted_contexts,MRR_context,MRR10_context,MRR5_context, HITS_context,HITS10_context = context_prediction(sessions, items_per_user, look_back, emb_dim, C, session_embds, item_embds, labels, centers,device,123+i,args)
        MRR_contexts.append(MRR_context)
        HITS_contexts.append(HITS_context)
        HITS10_contexts.append(HITS10_context)
        
        MRR5_contexts.append(MRR5_context)
        MRR10_contexts.append(MRR10_context)
        ...
        #return MRR, HITS,mrr5,hits5,mrr10
        MRR_now,HITS_now,mrr5_now,hits5_now,mrr10_now= item_prediction(sessions, items_per_user, look_back, emb_dim, C, item_embds, labels, predicted_contexts, device,123+i,args)
        
        MRR.append(MRR_now)
        MRR5.append(mrr5_now)
        MRR10.append(mrr10_now)
        HITS5.append(hits5_now)
        HITS.append(HITS_now)
        
        times.append(time.time()-start_time+input_time)

    print(np.average(times),file=open(output_path,"a"))
    print(MRR_contexts,HITS_contexts,HITS10_contexts,MRR10_contexts,MRR5_contexts, file=open(output_path, "a"))
    ...
    print(MRR,HITS,MRR5,MRR10,HITS5,file=open(output_path, "a"))
    print('MRR_average = {}\tMRR_std_dev = {}'.format(np.average(MRR),np.std(MRR)),file=open(output_path,"a"))
    print('MRR5_average = {}\tMRR5_std_dev = {}'.format(np.average(MRR5),np.std(MRR5)),file=open(output_path,"a"))
    print('MRR10_average = {}\tMRR10_std_dev = {}'.format(np.average(MRR10),np.std(MRR10)),file=open(output_path,"a"))
    print('Recall@5_average = {}\tRecall@5_std_dev = {}'.format(np.average(HITS5),np.std(HITS5)),file=open(output_path,"a"))
