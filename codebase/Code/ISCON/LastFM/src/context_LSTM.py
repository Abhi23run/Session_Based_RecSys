...
class context_LSTM(nn.Module):
    ...
    def compute_metric(self,test,test_users,test_items,test_contexts,predicted_contexts):
        ...
        hits5,mrr5,mrr10 = 0,0,0
        
        MRR,HITS,HITS10,hits5,mrr5,mrr10 = 0,0,0,0,0,0
        ...
        with torch.no_grad():
            for iteration in range(int(test_num / self.batch_size) + 1):
                ...
                for i in range(ed_idx-st_idx):
                    ...
                    MRR += 1/rank
                    HITS += (1 if rank <= 5 else 0)
                    hits5 += (1 if rank <= 5 else 0)
                    HITS10 += (1 if rank<=10 else 0)
                    mrr5 += (1/rank if rank <=5 else 0)
                    mrr10 += (1/rank if rank <=10 else 0)
        ...
        mrr5 /= test_num
        mrr10 /= test_num
        return MRR,HITS,HITS10, hits5, mrr5, mrr10
    ...
    def traintest(self, train, val, test, epochs,predicted_contexts):
        ...
        patience = 5
        best_loss,best_MRR,best_mrr5,best_mrr10,best_hits5,best_HITS,best_HITS10 = 0,0,0,0,0,0,0
        for epoch in range(epochs):
            ...
            if epoch % 1 == 0:
                 #return MRR,HITS,HITS10, hits5, mrr5, mrr10
                MRR_val,HITS_val,HITS10_val, hits5_val, mrr5_val,mrr10_val= self.compute_metric(val,val_users,val_items,val_contexts,predicted_contexts)
                MRR_test,HITS_test,HITS10_test,hits5_test, mrr5_test, mrr10_test = self.compute_metric(test,test_users,test_items,test_contexts,predicted_contexts)
                print("Epoch {}\tTrain Loss: {}\tVal HITS@10: {}\t Val HITS@5: {}\t Val MRR@5: {}\t Val MRR@10: {}\t Elapsed time: {}\tMRR_context = {}\tHITS_context = {}".format(epoch, train_loss,HITS10_val,hits5_val,mrr5_val, mrr10_val, time.time() - start_time,MRR_test,HITS_test))
                if best_loss<HITS10_val:
                    best_loss = HITS10_val
                    best_epoch = epoch
                    best_MRR,best_HITS,best_HITS10 = MRR_test,HITS_test,HITS10_test
                    
                    best_mrr5, best_mrr10, best_hits5 = mrr5_test, mrr10_test,hits5_test
                    
                    MRR_train,HITS_train,HITS10_train, hits5_train, mrr5_train, mrr10_train = self.compute_metric(train,train_users,train_items,train_contexts,predicted_contexts)
        ...
        print("[Context Prediction]\tEpoch {}\tTest_MRR = {}\t Test_MRR@5 ={}\t Test MRR@10 = {}\tTest_Recall@5 = {}\tTest_Recall@10 = {}".format(best_epoch, best_MRR,best_mrr5, best_mrr10,best_hits5,best_HITS10))
        return final_predicted_contexts,best_MRR,best_mrr10, best_mrr5,best_HITS,best_HITS10
